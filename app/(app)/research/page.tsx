  ...
  while (remaining.length > 0) {
    ...
    if (matches.length > 0 && matches[0]?.match.index !== undefined) {
      ...
      if (m.type === 'bold')
        ...
        parts.push(
          <code key={key++} className="px-1 py-0.5 rounded bg-accent/50 text-[12px]">
            {m.match[1]}
          </code>,
        );
        ...
        {elements.map((el, i) => {
          if (el.type === 'h1') {
            return (
              <h1
                key={i}
                className="text-2xl font-normal tracking-[0.05em] font-serif text-center pb-2"
                style={{ color: '#5C3018' }}
              >
          ...
          if (el.type === 'h2') {
            return (
              <h2
                key={i}
                className="text-lg font-normal tracking-[0.06em] font-serif pt-6 pb-1"
                style={{ color: '#5C3018' }}
              >
                {renderInline(el.content)}
              </h2>
          ...
          if (el.type === 'h3') {
            return (
              <h3
                key={i}
                className="text-[15px] font-medium pt-3 pb-0.5"
                style={{ color: '#7A5438' }}
              >
                {renderInline(el.content)}
              </h3>
          ...
          if (el.type === 'quote') {
            return (
              <blockquote
                ...
                style={{ borderColor: '#C4A06040', color: '#8A7A5A' }}
              >
                {renderInline(el.content)}
              </blockquote>
            );
