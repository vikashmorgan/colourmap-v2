...
export default function LifeTimeline() {
  ...
  function handleDropOnYear(year: number) {
    ...
    const updated = events.map((e) => e.id === dragId.current ? { ...e, year } : e).sort((a, b) => a.year - b.year);
    save(updated);
    dragId.current = null;
    setDropYear(null);
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={() => setAddingYear(addingYear !== null ? null : currentYear)}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all"
            style={{ color: addingYear !== null ? '#5C3018' : '#C4A06080', background: addingYear !== null ? '#5C301810' : 'transparent', border: '1px solid #C4A06015' }}>
          ...
          <div className="flex gap-0.5 rounded-lg overflow-hidden border border-border/30">
            {(['vertical', 'horizontal'] as const).map((o) => (
              <button key={o} type="button" onClick={() => setOrientation(o)}
                ...
                style={{ background: orientation === o ? '#C4A06020' : 'transparent', color: orientation === o ? '#5C3018' : '#5C301830' }}>
                {o === 'vertical' ? 'V' : 'H'}
              </button>
            ))}
          </div>
      ...
      {showSettings && (
        <div className="flex items-center gap-4 px-3 py-3 rounded-xl border border-border/30 animate-in fade-in duration-150">
          ...
          <input type="number" value={birthYear} min={1920} max={currentYear}
            onChange={(e) => {
      ...
      {orientation === 'vertical' && (
        <div className="space-y-0">
          {[...years].reverse()
            .filter((year) => showAllYears || year === currentYear || (eventsByYear.get(year) || []).length > 0)
            .map((year) => {
            ...
            return (
              <div key={year} className={`flex gap-4 group/year ${dropYear === year ? 'bg-[#C4A06010]' : ''}`}
                onDragOver={(e) => { e.preventDefault(); setDropYear(year); }}
                onDrop={() => handleDropOnYear(year)}>
                {/* Year label + timeline spine */}
                <div className="flex flex-col items-center w-14 shrink-0">
                  <div className="flex items-center gap-1.5">
                    <span className="text-sm font-medium tabular-nums"
                      style={{ color: isCurrent ? '#D4605A' : hasEvents ? '#5C301880' : '#5C301820' }}>
                      {year}
                    </span>
                    <span className="text-[10px] text-muted-foreground/25">{age}</span>
                  </div>
                  <div className="flex-1 w-px min-h-[20px]" style={{ background: hasEvents ? '#C4A06030' : '#C4A06008' }} />
                </div>
                ...
                <div className="flex-1 pb-2 min-h-[36px] flex items-start gap-2 flex-wrap pt-0.5">
                  ...
                  {yearEvents.map((event) => {
                    ...
                    if (isExpanded) {
                      return (
                        <div key={event.id}
                          ...
                          style={{ borderColor: `${event.color}30`, background: `${event.color}06` }}>
                          <div className="flex items-start justify-between">
                            <div className="flex items-center gap-2.5">
                              <div className="h-4 w-4 rounded-full" style={{ background: event.color, opacity: 0.8 }} />
                              <span className="text-base font-semibold font-serif" style={{ color: event.color }}>{event.title}</span>
                            </div>
                            <button type="button" onClick={() => setExpandedId(null)}
                              className="text-xs text-muted-foreground/30 hover:text-muted-foreground px-1">x</button>
                          </div>
                          {catObj && (
                            <span className="inline-block text-[10px] px-2.5 py-1 rounded-full"
                              style={{ background: `${catObj.color}15`, color: catObj.color, border: `1px solid ${catObj.color}20` }}>
                              {catObj.label}
                            </span>
                          )}
                          {event.note && (
                          ...
                          <div className="flex justify-end pt-2">
                            <button type="button" onClick={() => deleteEvent(event.id)}
                              className="text-[10px] text-muted-foreground/25 hover:text-destructive transition-colors">delete</button>
                          </div>
                        </div>
                      );
                    }
                    ...
                    return (
                      <button key={event.id} type="button" onClick={() => setExpandedId(event.id)}
                        draggable
                        onDragStart={() => { dragId.current = event.id; }}
                        onDragEnd={() => { dragId.current = null; setDropYear(null); }}
                        ...
                        <div className="h-3 w-3 rounded-full shrink-0" style={{ background: event.color, opacity: 0.8 }} />
                        <span className="text-xs font-medium" style={{ color: `${event.color}cc` }}>{event.title}</span>
                      </button>
                    );
                  })}
                  ...
                  {/* Add button */}
                  {!isAdding && (
                    <button type="button" onClick={() => setAddingYear(year)}
          ...
          {!showAllYears && years.length > 5 && (
            <button type="button" onClick={() => setShowAllYears(true)}
          ...
          {showAllYears && (
            <button type="button" onClick={() => setShowAllYears(false)}
              className="w-full text-[10px] text-muted-foreground/30 hover:text-muted-foreground/50 transition-colors py-2 text-center">
              Show less
            </button>
          )}
      ...
      {orientation === 'horizontal' && (
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-end gap-2">
            <button type="button" onClick={() => setYearWidth(Math.max(24, yearWidth - 12))}
              className="h-5 w-5 flex items-center justify-center rounded text-xs text-muted-foreground/40 hover:text-muted-foreground border border-border/30">-</button>
            <span className="text-[9px] text-muted-foreground/30">{yearWidth < 40 ? 'compact' : yearWidth > 64 ? 'wide' : 'normal'}</span>
            <button type="button" onClick={() => setYearWidth(Math.min(100, yearWidth + 12))}
              className="h-5 w-5 flex items-center justify-center rounded text-xs text-muted-foreground/40 hover:text-muted-foreground border border-border/30">+</button>
          </div>
          <div className="overflow-x-auto pb-3 scrollbar-none">
          <div className="flex items-end" style={{ minWidth: Math.max(years.length * yearWidth, 400) }}>
            {years.map((year) => {
              const yearEvents = eventsByYear.get(year) || [];
              const isAdding = addingYear === year;
              const isCurrent = year === currentYear;
              ...
              return (
                <div key={year} className="flex flex-col items-center group/col" style={{ width: yearWidth }}>
                    ...
                    {yearEvents.map((event) => {
                      ...
                      return (
                        <div key={event.id} className="relative">
                          <button type="button" onClick={() => setExpandedId(isExpanded ? null : event.id)}
                            className="h-4 w-4 rounded-full transition-all hover:scale-150"
                            style={{ background: event.color, opacity: 0.8 }}
                            title={event.title} />
                          {isExpanded && (
                            <div className="absolute bottom-7 left-1/2 -translate-x-1/2 z-50 w-56 rounded-2xl border border-border bg-card shadow-lg p-4 space-y-2 animate-in fade-in duration-150"
                              style={{ borderColor: `${event.color}30` }}>
                              <div className="flex items-start justify-between">
                                <div className="flex items-center gap-2">
                                  <div className="h-3 w-3 rounded-full" style={{ background: event.color }} />
                                  <span className="text-sm font-semibold font-serif" style={{ color: event.color }}>{event.title}</span>
                                </div>
                                <button type="button" onClick={(e) => { e.stopPropagation(); setExpandedId(null); }}
                                  className="text-xs text-muted-foreground/30 hover:text-muted-foreground px-1">x</button>
                              </div>
                              {event.note && <p className="text-xs text-muted-foreground leading-relaxed">{event.note}</p>}
                              <div className="flex justify-between">
                                <span className="text-[10px] text-muted-foreground/30">{event.year}</span>
                                <button type="button" onClick={() => deleteEvent(event.id)}
                                  className="text-[10px] text-muted-foreground/25 hover:text-destructive">delete</button>
                              </div>
                            </div>
                          )}
                        </div>
                  ...
                  {/* Year label */}
                  <div className="w-px h-2" style={{ background: isCurrent ? '#D4605A40' : hasEvents ? '#C4A06025' : '#C4A06008' }} />
                  <button type="button" onClick={() => setAddingYear(isAdding ? null : year)}
                    className="text-[9px] mt-0.5 tabular-nums"
                    style={{ color: isCurrent ? '#D4605A' : hasEvents ? '#5C301860' : '#5C301815', fontWeight: isCurrent || hasEvents ? 600 : 400 }}>
        ...
        <div className="rounded-xl border border-border/40 p-4 space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="flex items-center justify-between">
            <p className="text-sm font-serif" style={{ color: '#5C3018' }}>
              {addingYear} <span className="text-[10px] text-muted-foreground/40 ml-1">age {addingYear - birthYear}</span>
            </p>
            <button type="button" onClick={() => { setAddingYear(null); setNewTitle(''); setNewNote(''); }}
              className="text-xs text-muted-foreground/30 hover:text-muted-foreground transition-colors">close</button>
          </div>

          <input type="text" value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter' && newTitle.trim()) addEvent(); }}
          ...
          {/* Add button */}
          <button type="button" onClick={addEvent} disabled={!newTitle.trim()}
