...
function Pill({ label, color, open, onToggle, count, children }: {
  label: string; color: string; open: boolean; onToggle: () => void; count?: number; children: React.ReactNode;
}) {
  return (
    <div>
...
export default function SegmentPills({ segment, color, doingQuestions, notes, onSaveAnswers, missions = [], hasWeaknessMap }: SegmentPillsProps) {
  ...
  const log: LogEntry[] = (() => { try { return JSON.parse(notes[`${segment}_log`] || '[]'); } catch { return []; } })();
  const actions: Action[] = (() => { try { return JSON.parse(notes[`${segment}_actions`] || '[]'); } catch { return []; } })();
  const weaknesses: WeaknessEntry[] = (() => { try { return JSON.parse(notes[`${segment}_weaknesses`] || '[]'); } catch { return []; } })();
  ...
  return (
    <div className="space-y-0.5">
      {/* 1. Questions */}
      <Pill label="Questions" color={color} open={openPill === 'questions'} onToggle={() => toggle('questions')}>
        {doingQuestions ? (
          <QuestionPicker questions={doingQuestions} segment={segment} notes={notes} color={color} onSaveAnswers={onSaveAnswers} />
        ) : (
          <QuestionField qKey={`overview_${segment}`} label=""
            placeholder="Write your thoughts..." initial={notes[segment] || ''} color={color}
      ...
      {hasWeaknessMap && (
        <Pill label="Battle Map" color={color} open={openPill === 'battlemap'} onToggle={() => toggle('battlemap')}
          count={weaknesses.length}>
          <WeaknessMap weaknesses={weaknesses} color={color} saving={saving} onSave={saveWeaknesses} />
        </Pill>
      )}

      {/* 3. Notes */}
      <Pill label="Notes" color={color} open={openPill === 'notes'} onToggle={() => toggle('notes')} count={log.length}>
        <div className="space-y-3">
          <div className="flex gap-2">
            ...
            />
            {newEntry.trim() && (
              <button type="button" onClick={addLogEntry} disabled={saving}
                className="text-[10px] font-medium px-2" style={{ color }}>
                {saving ? '...' : 'Add'}
              </button>
            )}
          </div>
            ...
            <div className="space-y-1.5 max-h-48 overflow-y-auto">
              {log.map((entry, i) => (
                <div key={i} className="flex gap-2 text-xs group">
                  <span className="text-muted-foreground/40 shrink-0 text-[10px] pt-0.5">{formatDate(entry.date)}</span>
                  <span className="text-muted-foreground flex-1">{entry.text}</span>
                  <button type="button" onClick={() => removeLogEntry(i)}
                    className="text-[9px] text-muted-foreground/0 group-hover:text-muted-foreground/40 hover:!text-destructive transition-colors shrink-0">×</button>
                </div>
              ))}
            </div>
          ) : (
      ...
      {/* 3. Action Plan */}
      <Pill label="Action Plan" color={color} open={openPill === 'actions'} onToggle={() => toggle('actions')}
        count={actions.filter(a => !a.done).length}>
        <ActionPlan actions={actions} color={color} saving={saving} onSave={saveActions} segment={segment} missions={missions} />
      </Pill>
    </div>
  );
}

function WeaknessMap({ weaknesses, color, saving, onSave }: {
  weaknesses: WeaknessEntry[]; color: string; saving: boolean; onSave: (w: WeaknessEntry[]) => void;
}) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-muted-foreground/60 text-center leading-relaxed">
        Map what makes you feel weak or powerless.<br />Then write how you can face it.
      </p>
                ...
                <div className="flex items-center gap-2 group">
                  <button type="button"
                    onClick={() => {
                      ...
                      else { setExpandedIdx(i); setEditResponse(w.response); }
                    }}
                  ...
                  <button type="button" onClick={() => remove(i)}
                    className="text-xs text-muted-foreground/0 group-hover:text-muted-foreground/40 hover:!text-destructive transition-colors shrink-0">×</button>
                </div>
                {isOpen && (
                  <div className="mt-3 pl-2 space-y-2 animate-in fade-in duration-150">
                    <p className="text-xs italic text-muted-foreground/60">How can you face this? What would you tell yourself?</p>
                    <textarea
                      value={editResponse}
                    ...
                    {hasResponse && (
                      <div className="rounded-xl p-3" style={{ background: `${color}06` }}>
                        <p className="text-xs leading-relaxed" style={{ color }}>{w.response}</p>
                      </div>
                    )}
                    <div className="flex justify-end">
                      <button type="button" onClick={() => saveResponse(i)} disabled={savingResponse}
      ...
      <div className="flex gap-2">
        <input
          ...
          onKeyDown={e => { if (e.key === 'Enter') addWeakness(); }}
        ...
        />
        {newWeakness.trim() && (
          <button type="button" onClick={addWeakness} disabled={saving}
            className="text-xs font-medium px-3" style={{ color }}>
            {saving ? '...' : 'Add'}
          </button>
        )}
      </div>
...
function ActionPlan({ actions, color, saving, onSave, segment, missions }: {
  actions: Action[]; color: string; saving: boolean; onSave: (a: Action[]) => void; segment: string; missions: MissionLink[];
}) {
  ...
  async function sendToCockpit(action: Action, idx: number) {
    ...
    const label = action.firstStep
      ? `${action.what} → ${action.firstStep}`
      : action.what;
  ...
  function add() {
    if (!what.trim()) return;
    const linkedLabel = linkTo === 'daily' ? 'Daily'
      : linkTo.startsWith('mission:') ? missions.find(m => m.id === linkTo.replace('mission:', ''))?.title
      : undefined;
    onSave([...actions, {
      what: what.trim(), by, firstStep: firstStep.trim(), done: false,
  ...
  return (
    <div className="space-y-3">
      {actions.length > 0 && (
        <div className="space-y-2">
          {actions.map((a, i) => (
            <div key={i} className={`rounded-xl border px-3 py-2.5 space-y-1 group ${
              ...
              <div className="flex items-start gap-2">
                <button type="button" onClick={() => toggleDone(i)}
                  ...
                  style={a.done ? { borderColor: color, backgroundColor: color, color: 'white' } : undefined}>
                  {a.done ? '✓' : ''}
                </button>
                <div className="flex-1 min-w-0">
                  <p className={`text-xs font-medium ${a.done ? 'line-through' : ''}`}>{a.what}</p>
                  {a.firstStep && (
                    <p className="text-[10px] text-muted-foreground mt-0.5">First step: {a.firstStep}</p>
                  )}
                  {a.linkedLabel && (
                    <p className="text-[9px] mt-0.5" style={{ color }}>↳ {a.linkedLabel}</p>
                  )}
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  {a.by && (
                    <span className="text-[9px] text-muted-foreground/50">by {new Date(a.by).toLocaleDateString([], { month: 'short', day: 'numeric' })}</span>
                  )}
                  {!a.done && !a.inCockpit && (
                    <button type="button" onClick={() => sendToCockpit(a, i)}
                      className="text-[9px] text-muted-foreground/50 hover:text-foreground transition-colors"
                      title="Track in cockpit">→ cockpit</button>
                  )}
                  {a.inCockpit && (
                    <span className="text-[9px]" style={{ color: '#C4A060' }}>✓ cockpit</span>
                  )}
                  <button type="button" onClick={() => remove(i)}
                    className="text-[9px] text-muted-foreground/0 group-hover:text-muted-foreground/40 hover:!text-destructive transition-colors">×</button>
                </div>
              </div>
            </div>
          ))}
      ...
      {adding ? (
        <div className="rounded-xl border border-border p-3 space-y-2 animate-in fade-in duration-150">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <button type="button" onClick={() => setLinkTo(linkTo === 'daily' ? '' : 'daily')}
              className={`rounded-full px-2 py-0.5 text-[9px] transition-colors ${
                linkTo === 'daily' ? 'font-medium' : 'text-muted-foreground/50'
              }`}
              style={linkTo === 'daily' ? { background: `${color}15`, color, border: `1px solid ${color}30` } : { border: '1px solid transparent' }}>
              Daily Tasks
            </button>
            {missions.map(m => {
              ...
              return (
                <button key={m.id} type="button" onClick={() => setLinkTo(active ? '' : val)}
                  className={`rounded-full px-2 py-0.5 text-[9px] transition-colors ${
                    active ? 'font-medium' : 'text-muted-foreground/50'
                  }`}
                  style={active ? { background: `${color}15`, color, border: `1px solid ${color}30` } : { border: '1px solid transparent' }}>
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={add} disabled={!what.trim() || saving}
        ...
        <button type="button" onClick={() => setAdding(true)}
          ...
          <span className="flex h-4 w-4 items-center justify-center rounded-full border border-border/40 text-[9px]">+</span>
...
function QuestionPicker({ questions, segment, notes, color, onSaveAnswers }: {
  questions: DoingQ[]; segment: string; notes: Record<string, string>; color: string;
  onSaveAnswers: (answers: Record<string, string>) => void;
}) {
        ...
        return (
          <div key={dq.key}>
            <button type="button"
              ...
              <p className="text-xs font-semibold" style={{ color }}>{dq.label}</p>
              {!isActive && answer && (
                <p className="text-[11px] text-muted-foreground mt-0.5 truncate">{answer}</p>
              )}
              {!isActive && !answer && (
                <p className="text-[11px] text-muted-foreground/30 mt-0.5 italic">{dq.placeholder}</p>
              )}
            </button>
            {isActive && (
...
function QuestionField({ qKey, label, placeholder, initial, color, onSaved }: {
  qKey: string; label: string; placeholder: string; initial: string; color: string; onSaved: (v: string) => void;
}) {
