...
const WHEELS = [
  // Order: [right, bottom, left, top] to match X-divided SVG paths
  {
    id: 'feeling',
    name: 'Feeling',
    segments: ['Presence', 'Body', 'Emotions', 'Attitude'],
    colors: ['#A8C8A0', '#8AB888'],
  },
  {
    id: 'doing',
    name: 'Doing',
    segments: ['Structure', 'Action', 'Resources', 'Direction'],
    colors: ['#E8C898', '#D4B078'],
  },
  {
    id: 'sharing',
    name: 'Sharing',
...
function _getDotColor(v: number) {
  return v <= 20
    ? '#D4605A'
    : v <= 40
      ? '#E0844A'
  ...
  switch (segment) {
    case 'Attitude':
      return {
        title: 'Attitude',
        wisdom:
          'Attitude is what you build when times get tough. It will help you walk through the storms and overcome the challenges of existence. Your attitude is not what happens to you — it is what you decide to carry.',
        question: '',
        actions: [],
        hasWeaknessMap: true,
        doingQuestions: [
          { key: 'story', label: 'Story', placeholder: 'What is the story you tell yourself?' },
          {
            key: 'selftalk',
            label: 'Self Talk',
            placeholder: 'How can you improve your self talk?',
          },
          { key: 'notwant', label: 'Shadow', placeholder: 'What do you not want to be?' },
          {
            key: 'direction',
            label: 'Direction',
            placeholder: 'What attitude do you want to carry?',
          },
          {
            key: 'resources',
            label: 'Resources',
            placeholder: 'What helps you shift your mindset? (people, habits, reminders)',
          },
          {
            key: 'structure',
            label: 'Structure',
            placeholder: 'What daily practice keeps your attitude in check?',
          },
          {
            key: 'action',
            label: 'Action',
            placeholder: 'One thing you can do right now to shift your state',
          },
        ],
      };
    case 'Body':
      return {
        title: 'Body',
        wisdom:
          'Your body is the vessel. It carries everything — your dreams, your fears, your power. When you neglect it, everything suffers. When you listen to it, it tells you exactly what you need.',
        question: '',
        actions: [],
        doingQuestions: [
          {
          ...
          },
          {
            key: 'direction',
            label: 'Direction',
            placeholder: 'Where do you want your body to be?',
          },
          {
            key: 'resources',
            label: 'Resources',
            placeholder: 'What do you already have? (gym, habits, knowledge)',
          },
    ...
    case 'Emotions': {
      ...
      return {
        title: 'Emotions',
        wisdom:
          'Emotions allow you to be human, alive. They connect the body to the mind and require you to listen in order to find the power of your highest transformation. Every emotion is information — not an enemy.',
    ...
    case 'Presence':
      return {
        title: 'Presence',
        wisdom:
          'Presence is where life actually happens. Not in the past you replay, not in the future you fear — but here, now. The quality of your attention is the quality of your life.',
        ...
        doingQuestions: [
          {
            key: 'attention',
            label: 'Attention',
            placeholder: 'What is pulling your attention away?',
          },
          {
            key: 'practice',
            label: 'Practice',
            placeholder: 'What brings you back to the present?',
          },
          {
            key: 'reflection',
            label: 'Reflection',
            placeholder: 'What did you notice today about your presence?',
          },
          {
            key: 'action',
            label: 'Action',
            placeholder: 'One presence practice to track over time',
          },
        ],
      };
    case 'Direction':
      return {
        title: 'Direction',
        wisdom:
          'Without direction, energy scatters. Direction is not about knowing every step — it is about knowing which horizon you are walking toward. Even one degree of clarity changes everything over time.',
        dataItems: visionText ? [visionText] : undefined,
        question: 'Do you know where you are going?',
        actions: [
          'Review your 6-month vision',
          'What matters most this week?',
          'Write one clear priority for today',
        ],
        link: '/life-scan',
        linkLabel: 'Set your vision',
      };
    case 'Resources':
      return {
        title: 'Resources',
        wisdom:
          'You already have more than you think. Your skills, your experience, your relationships — these are your resources. The question is not what you lack but what you are not yet using.',
        status: strengths?.length
          ? `${strengths.length} strength${strengths.length !== 1 ? 's' : ''} mapped`
          : undefined,
        dataItems: strengths?.slice(0, 4),
        question: 'What skill are you not using enough?',
        actions: [
          'Double down on one strength',
          "Ask for help where you're weak",
          'Learn one new thing this week',
        ],
        link: '/life-scan',
        linkLabel: 'Map your strengths',
      };
    case 'Action': {
      const items: string[] = [];
      ...
      return {
        title: 'Action',
        wisdom:
          'Action is where intention meets reality. You do not need to feel ready. You need to start. The smallest step forward is worth more than the grandest plan that stays in your head.',
        status: activeMissions.length
    ...
    case 'Structure':
      return {
        title: 'Structure',
        wisdom:
          'Structure is freedom in disguise. Without it you drift, with it you build. The right routine does not cage you — it creates the container where your best work happens. Organisation is the foundation of clarity.',
        question: '',
        actions: [],
        doingQuestions: [
          { key: 'routine', label: 'Routine', placeholder: 'What routine is broken right now?' },
          {
            key: 'organisation',
            label: 'Organisation',
            placeholder: 'What area of your life needs organising?',
          },
          {
            key: 'systems',
            label: 'Systems',
            placeholder: 'What system would make your life easier?',
          },
          {
            key: 'discipline',
            label: 'Discipline',
            placeholder: 'What discipline are you avoiding?',
          },
    ...
    case 'Family':
      return {
        title: 'Family',
        wisdom:
          'Family is the mirror you did not choose. It shows you who you are, where you come from, and what patterns you carry. The work is not to change them — it is to change how you show up.',
        status: blockedMissions.length
          ? `${blockedMissions.length} mission${blockedMissions.length !== 1 ? 's' : ''} blocked`
          : undefined,
    ...
    case 'Love':
      return {
        title: 'Love',
        wisdom:
          'Love is not something you find — it is something you practice. It starts with how you treat yourself and ripples outward. The deepest courage is to love without armor.',
        ...
        link: '/life-scan',
        linkLabel: 'Who can you talk to?',
      };
    case 'Friends':
      return {
        title: 'Friends',
        wisdom:
          'Friends are the family you choose. They sharpen you, hold you accountable, and remind you who you are when you forget. But friendship requires showing up — not just when it is easy.',
        question: 'What friendship are you neglecting?',
        actions: [
          'Text one friend right now',
          'Plan something this week',
          'Show up for someone',
          'Be more honest in a conversation',
        ],
      };
    case 'Network':
      return {
        title: 'Network',
        wisdom:
          'Your network is your reach beyond yourself. It is not about collecting contacts — it is about meaningful exchanges. Who challenges you to grow? Who needs what you have to offer?',
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="cursor-pointer">
      {segColors.map((color, i) => {
        ...
        return (
          <path
            key={i}
            d={`M${cx},${cy} L${cx + r * Math.cos(sr)},${cy + r * Math.sin(sr)} A${r},${r} 0 0,1 ${cx + r * Math.cos(er)},${cy + r * Math.sin(er)} Z`}
            fill={color}
            opacity={isActive ? 0.85 : 0.5}
            onClick={() => onSegmentClick(wheel.segments[i])}
  ...
  return (
    <div className="space-y-2">
      ...
      <div
        ...
        onMouseMove={(e) => {
          if (e.buttons > 0) handleInteract(e.clientX, e.currentTarget.getBoundingClientRect());
        }}
        onTouchStart={(e) =>
          handleInteract(e.touches[0].clientX, e.currentTarget.getBoundingClientRect())
        }
        onTouchMove={(e) => {
          e.preventDefault();
          handleInteract(e.touches[0].clientX, e.currentTarget.getBoundingClientRect());
      ...
      <div className="flex items-center justify-center gap-4">
        <button
          ...
          onClick={() => {
            ...
            setWheelIndex((wheelIndex - 1 + 3) % 3);
          }}
          className="text-xl text-muted-foreground hover:text-foreground transition-colors px-8 py-16 -mr-6 cursor-pointer"
        >
          ‹
        </button>
        <div className="flex flex-col items-center gap-2">
          ...
          <WheelSVG
            ...
            onSegmentClick={(s) => {
              setActiveSegment(activeSegment === s ? null : s);
              setShowWisdom(false);
            }}
            size={220}
          />
        ...
        <button
          ...
          onClick={() => {
            setActiveSegment(null);
            setWheelIndex((wheelIndex + 1) % 3);
          }}
          className="text-xl text-muted-foreground hover:text-foreground transition-colors px-8 py-16 -ml-6 cursor-pointer"
        >
          ›
        </button>
      </div>
