...
export default function SoulMap({ data, onSelectTerritory }: { data: SoulMapData; onSelectTerritory?: (id: string) => void }) {
  ...
  return (
    <div className="space-y-2">
      <div className="rounded-2xl border border-border/30 overflow-hidden" style={{ background: '#F8F0E408' }}>
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 'auto', minHeight: 280 }}>
          ...
          {territories.map((t) => {
            ...
            return (
              <g key={t.id} className="cursor-pointer" onClick={() => onSelectTerritory?.(t.id)}>
                ...
                {/* Contour rings */}
                <circle cx={t.cx} cy={t.cy} r={t.r * 0.9} fill="none" stroke={mid} strokeWidth={0.3} opacity={0.2} />
                <circle cx={t.cx} cy={t.cy} r={t.r * 0.55} fill="none" stroke={dark} strokeWidth={0.2} opacity={0.25} />

                {/* Territory name */}
                <text
                  x={t.cx} y={t.cy - (t.items.length > 0 ? t.items.length * 2.5 + 1 : 1)}
                  textAnchor="middle"
          ...
          {/* Chapter at bottom */}
          {data.chapter && (
            <text x={50} y={H - 3} textAnchor="middle" fill="#5C3018" fontSize={2.5}
              fontFamily="var(--font-serif)" opacity={0.25} letterSpacing="1">
              {data.chapter.toUpperCase()}
            </text>
          )}

          {/* Archetype at top */}
          <text x={50} y={4} textAnchor="middle" fill="#5C3018" fontSize={2.5}
            fontFamily="var(--font-serif)" opacity={0.2} letterSpacing="0.8">
            {data.archetype}
          </text>
        </svg>
      </div>
